#!/usr/bin/perl
#TODO sending privmsg should also modify mood
#
...
sub event_message_own_private {
	...
	change_mood($nick, find_smiley($msg));
}
...
sub mood_show {
    ...
    if (ref $win && ($win->{type}) and ($win->{type} eq "CHANNEL" or $win->{type} eq "QUERY")) {
